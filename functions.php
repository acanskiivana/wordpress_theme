	...
	function starter_theme_setup() {
		...
		register_nav_menus(
			array(
				...
				'footer-links' => esc_html__( 'Footer links', 'starter-theme' )
			)
		);
...
function starter_theme_scripts() {
	wp_enqueue_style( 'starter-theme-style', get_stylesheet_uri(), array(), '1.256399891234578521351' );

	// wp_enqueue_style( 'custom-style', get_template_directory_uri() . '/css/custom-style.css', array(), '1.2354' );

	wp_enqueue_style( 'slick-css', get_template_directory_uri() . '/css/slick.css', array(), _S_VERSION );

	wp_enqueue_style( 'select2-css', get_template_directory_uri() . '/css/select2.css', array(), _S_VERSION );

	wp_enqueue_style( 'aos-css', get_template_directory_uri() . '/aos.css', array(), _S_VERSION );

	wp_enqueue_script( 'aos-js', 'https://unpkg.com/aos@2.3.1/dist/aos.js', array('jquery'), _S_VERSION, true );
	...
	if (  is_home() ) {
	    ...
	    // localize ajax 
	    wp_localize_script( 'ajax', 'ajaxUrl', array('ajaxurl' => admin_url( 'admin-ajax.php' )));
	}

	
	if (  is_page( 'Product' ) ) {
		wp_enqueue_script( 'contact-sales-js', get_template_directory_uri() . '/js/contact-sales.js', array('jquery'), '1.3', true );
	}

	wp_enqueue_script( 'main', get_template_directory_uri() . '/js/main.js', array('jquery'), '1.2563981', true );
...
/**
 * get infinite scroll and ajax filter posts by categories.
 */
// require get_template_directory() . '/inc/infinite-scroll.php';

/**
 * ajax pagination filter posts by categories and search field.
 */
require get_template_directory() . '/inc/ajax-function.php';


add_image_size( 'blog_block', 375, 240 ); // hard crop mode
...
// Redirect page /products/ to the home page 
function quadlayers_redirect() {
	...
	case '/856-2':
	$urlto = home_url('/energiekontor-ag-germany');
	break;
